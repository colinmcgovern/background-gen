...
int main()
{
	...
	for(size_t i=0;i<width;i++){
		for(size_t j=0;j<height;j++){
			...
			if(noise[i][j]<0.25){
				test[index] = palette[0][0];
				index++;
				test[index] = palette[0][1];
				index++;
				test[index] = palette[0][2];
				index++;
				test[index] = palette[0][3];
				index++;
			}else if(noise[i][j]<0.5){
				test[index] = palette[1][0];
				index++;
				test[index] = palette[1][1];
				index++;
				test[index] = palette[1][2];
				index++;
				test[index] = palette[1][3];
				index++;
			}if(noise[i][j]<0.75){
				test[index] = palette[0][0];
				index++;
				// test[index] = palette[2][1];
				// index++;
				// test[index] = palette[2][2];
				// index++;
				// test[index] = palette[2][3];
				// index++;
			}
			// }else{
			// 	test[index] = palette[3][0];
			// 	index++;
			// 	test[index] = palette[3][1];
			// 	index++;
			// 	test[index] = palette[3][2];
			// 	index++;
			// 	test[index] = palette[3][3];
			// 	index++;
			// }

			// for(size_t k=0;k<4;k++){

			// 	test[index] = uint((noise[i][j]+0.5)*255);

			// 	cout << index << " " << uint((noise[i][j]+0.5)*255) << endl; //del
